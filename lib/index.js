var _ = require('underscore'),
	...
	filterEntries = function(options) { //names, appName, appVersion, environmentName, associationPriority, entries
		options = options || {};
		options.associationPriority = options.associationPriority || 'app';
		if (!options.names || options.names.length < 1) {
			return {};
		}
		if (!options.appName || !options.appVersion || !options.environmentName) {
		...
		for (prop in groups) {
			if (groups.hasOwnProperty(prop)) {
				if (groups[prop].length === 1) {
					config[prop] = groups[prop][0].value;
				}
				else {
					config[prop] = pickOne(groups[prop], options.appName, options.appVersion, options.environmentName, associationPriority).value;
				}
			}
		}
		return config;
